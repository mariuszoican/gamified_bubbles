...
mbo=pd.read_csv("data/trader_bridge_app_custom_export_mbo_2026-03-06.csv") # load mbo data
mbo=mbo[mbo.trading_session_uuid==session_code] # only keep relevant session
mbo['direction']=np.where(mbo['side']=='bid','buy','sell')

mbp=pd.read_csv("data/trader_bridge_app_custom_export_mbp1_2026-03-06.csv") # load mbo data
